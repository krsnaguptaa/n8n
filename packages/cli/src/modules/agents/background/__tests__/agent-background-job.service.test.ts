...
describe('registerWorkflowJob', () => {
	...
	it('returns duplicate when the execution is already registered', async () => {
		const { service, jobRepository } = setup();
		...
		expect(receipt).toEqual({ status: 'duplicate', existingJobId: 'wf-existing' });
	});

	it('returns duplicate for a replayed registration even after the job settled', async () => {
		const { service, jobRepository } = setup();
		jobRepository.insertWorkflowJobOrGetExisting.mockResolvedValue({
			inserted: false,
		...
		expect(receipt).toEqual({ status: 'duplicate', existingJobId: 'wf-settled' });
	});
});
