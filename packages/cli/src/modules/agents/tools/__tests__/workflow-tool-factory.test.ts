...
describe('workflow tool → background job handoff', () => {
	...
	it('hands back the existing job when the execution is already registered', async () => {
		setPersistence({ status: 'waiting' });
		const jobService = setJobService();
		jobService.registerWorkflowJob.mockResolvedValue({
			status: 'duplicate',
		...
		expect(result).toMatchObject({ status: 'running_in_background', jobId: 'job-existing' });
