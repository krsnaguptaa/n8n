...
describe('SystemTaskTimer', () => {
	...
	function createTimer(schedule: SystemTaskSchedule) {
		return new SystemTaskTimer(resolveSystemTaskSchedule(schedule, 'UTC'), onFire, onPlanError);
	}
	...
	it('coalesces the occurrences a stalled process slept through', () => {
		...
		const timer = new SystemTaskTimer(
			resolveSystemTaskSchedule({ kind: 'interval', intervalSeconds: 60 }, 'UTC'),
			onFire,
			onPlanError,
			() => clock,
		);
...
describe('resolveSystemTaskSchedule', () => {
	it('resolves a cron schedule with no timezone to the instance default', () => {
		const resolved = resolveSystemTaskSchedule(
	...
	it('keeps an explicit timezone', () => {
		...
		expect(resolved).toMatchObject({ timezone: 'UTC' });
	});

	it('passes an interval schedule through', () => {
		const schedule: SystemTaskSchedule = { kind: 'interval', intervalSeconds: 30 };

		expect(resolveSystemTaskSchedule(schedule, 'UTC')).toEqual(schedule);
	});
});
