...
describe('GmailTrigger', () => {
	...
	describe('v1.4 - multi-page backlog pagination', () => {
		...
		// Budget tests prove a request did NOT happen, so their unconsumed
		...
		it('should give up holding when a capped window makes no progress', async () => {
			...
			// pages can never progress past them. Holding on would repeat this tick
			// forever: no backlog progress, no new mail, no warning. Two ticks
			// already found nothing, so this one must give up — advance and start
			// fresh.
			const initialTimestamp = 1000000;
