...
export class LmChatDatabricks implements INodeType {
	...
	async supplyData(this: ISupplyDataFunctions, itemIndex: number): Promise<SupplyData> {
		...
		// This node mints the token itself; an http host would send the client
		// secret and bearer token in cleartext
		if (!credential.host.startsWith('https://')) {
			throw new NodeOperationError(this.getNode(), 'Databricks host must use https');
		}
		...
		const configuration: ClientOptions = {
			baseURL,
			fetch: createDatabricksFetch(getDatabricksTokenProvider(this.getNode(), credential)),
			fetchOptions: {
